...
def test_get_context_parses_payload() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                ...
                "examples": [{"meeting_summary": "resumen", "estimation": "estimación"}],
                "transcription_min_length": 10,
                "transcription_max_length": 50000,
                "llm_configured": True,
            },
        )
    ...
    assert context.examples[0].meeting_summary == "resumen"
    assert context.examples[0].estimation == "estimación"
    assert context.transcription_min_length == 10
    assert context.transcription_max_length == 50000
...
def test_stream_estimation_yields_tokens_and_metrics() -> None:
    ...
    chunks = list(client.stream_estimation("transcripción", metrics, client=_client(handler)))
...
def test_stream_estimation_provider_error_event_raises() -> None:
    ...
    stream = client.stream_estimation("transcripción", client=_client(handler))
...
def test_stream_estimation_maps_error_statuses(status_code: int, expected: type[ApiError]) -> None:
    ...
    with pytest.raises(expected):
        list(client.stream_estimation("transcripción", client=_client(handler)))
